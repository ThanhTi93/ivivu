            ...
            <div class="right" id="main-content">
                ...
                <div >
                    <div class="container mt-5">
                        ...
                        <form action="editlocation" method="post" enctype="multipart/form-data" class="p-4 shadow-lg rounded bg-light mt-3">
                            <div class="row row-cols-1 row-cols-md-2">
                                <div class="col">
                                    <input type="text" value="${location.id}" class="form-control d-none" name="id" placeholder="Nhập tên địa điểm" >
                                    <div class="mb-3">
                                        <label class="form-label">Name Location</label>
                                        <input type="text" value="${location.name}" class="form-control" name="name" placeholder="Nhập tên địa điểm" required>
                                    </div>                                 
                                    <div class="mb-3">
                                        <label class="form-label">Description</label>
                                        <textarea  class="form-control" name="description" rows="3" placeholder="Nhập mô tả" required>${location.description}</textarea>
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Category</label>
                                        <select class="form-select" name="categoryId" aria-label="Default select example">
                                            <option disabled>Vui lòng chọn Category</option>
                                            <c:forEach var="o" items="${listCategory}">
                                                <option value="${o.id}" ${o.id == location.category_id ? "selected" : ""}>
                                                    ${o.name}
                                                </option>
                                            </c:forEach>
                                        </select>
                                ...
                                <div class="col">
                                    <div class="mb-3">
                                        ...
                                        <div class="w-50 m-auto my-2">
                                            <img id="previewImage" src="${location.imgUrl}" class="mt-3" >
                                        </div>                                     
